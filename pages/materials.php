...
if ($sort === 'value') {
    usort($materialRows, function ($a, $b) {
        $valueA = unit_price_from_description($a['description']) * (int) $a['total_quantity'];
        $valueB = unit_price_from_description($b['description']) * (int) $b['total_quantity'];
        return $valueB <=> $valueA;
    });
}
$materialCategories = all_rows(
...
foreach ($materialRows as $row) {
    $totalValue += unit_price_from_description($row['description']) * (int) $row['total_quantity'];
}
$totalCategories = count(array_unique(array_filter(array_map(function ($row) {
...
<head>
  ...
  <title>Campus Materials • MSU-MCEST CEMS</title>
...
<aside class="sidebar hidden md:flex md:flex-col w-64 bg-navy text-white fixed inset-y-0 left-0 z-30">
  ...
  <nav class="flex-1 py-4 text-sm">
    ...
    <a href="materials.php" class="nav-link active">Campus Materials</a>
  ...
  <div class="md:ml-64 main-shift page-fade">
    <header class="bg-white border-b border-gray-200 sticky top-0 z-10">
      <div class="px-6 py-4 flex flex-wrap items-center gap-4">
        ...
        <div class="min-w-0">
          <h1 class="text-lg font-semibold text-navy leading-tight whitespace-nowrap">Campus Materials</h1>
          <p class="text-xs text-gray-500">School property and materials inventory</p>
        </div>
        <div class="ml-auto flex items-center gap-2 flex-wrap justify-end"><input class="input max-w-xs" placeholder="Search materials..." data-search-target="#materialsTable" /><a class="btn btn-outline btn-sm" href="materials.php?<?php echo h(http_build_query(array_merge($_GET, ['export' => 'csv']))); ?>">Export CSV</a><button class="btn btn-primary btn-sm" data-modal-open="addMaterialModal">+ Add Material</button></div>
      </div>
    </header>
    ...
    <main class="p-6 space-y-6">
      <section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="card">
        <div class="card-body">
          <p class="text-xs uppercase tracking-wider text-gray-500">Total Materials</p>
          <p class="mt-2 text-2xl font-semibold text-navy"><?php echo $totalMaterials; ?></p>
          <p class="text-xs text-gray-400 mt-1">Distinct items</p>
        </div>
      </div>
        <div class="card">
        <div class="card-body">
          <p class="text-xs uppercase tracking-wider text-gray-500">Total Quantity</p>
          <p class="mt-2 text-2xl font-semibold text-navy"><?php echo $totalQuantity; ?></p>
          <p class="text-xs text-gray-400 mt-1">Units on hand</p>
        </div>
      </div>
        <div class="card">
        <div class="card-body">
          <p class="text-xs uppercase tracking-wider text-gray-500">Total Value</p>
          <p class="mt-2 text-2xl font-semibold text-navy"><?php echo money($totalValue); ?></p>
          <p class="text-xs text-gray-400 mt-1">PHP</p>
        </div>
      </div>
        <div class="card">
        <div class="card-body">
          <p class="text-xs uppercase tracking-wider text-gray-500">Categories</p>
          <p class="mt-2 text-2xl font-semibold text-navy"><?php echo $totalCategories; ?></p>
          <p class="text-xs text-gray-400 mt-1">Distinct groups</p>
        </div>
      </div>
      </section>
        ...
        <div class="card-header">
          <form method="GET" class="flex items-center gap-2">
            ...
            <select class="select max-w-[10rem]" name="sort" onchange="this.form.submit()">
              ...
              <option value="value" <?php echo $sort === 'value' ? 'selected' : ''; ?>>Total value</option>
            </select>
          </form>
        </div>
              ...
              ?>
                <tr data-searchable>
                  ...
                  <td class="text-right space-x-2">
                    ...
                    <form method="POST" action="../process/materials.php" class="inline" onsubmit="return confirm('Delete this material?');">
    ...
    <div class="modal" id="addMaterialModal">
      <div class="modal-card">
        <div class="card-header">
          <h3 class="font-semibold text-navy">Add Material</h3>
          <button data-modal-close class="text-gray-400 hover:text-gray-700">✕</button>
        </div>
        <form method="POST" action="../process/materials.php" class="card-body grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="hidden" name="id" value="" />
          <div><label class="label">Material name</label><input class="input" name="material_name" required /></div>
  ...
  <script>
    ...
    document.querySelectorAll('[data-modal-open="addMaterialModal"]:not([data-edit-material])').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var form = document.querySelector('#addMaterialModal form');
